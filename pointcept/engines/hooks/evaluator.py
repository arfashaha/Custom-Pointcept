...
class SemSegEvaluator(HookBase):
    ...
    def eval(self):
        ...
        # ---- Additional Metrics: Dice, Precision, Recall ----
        TP = intersection
        FP = union - intersection
        FN = target - intersection
        TN = sum(target) - TP - FP - FN  # For MCC, approximate (may not always be correct for multi-class)
        # For each class:
        ...
        self.trainer.logger.info(
            "Val result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f} | Dice/F1 {:.4f} | Precision {:.4f} | Recall {:.4f}".format(
                m_iou, m_acc, all_acc, mean_dice, mean_precision, mean_recall
            )
        )
        for i in range(self.trainer.cfg.data.num_classes):
            self.trainer.logger.info(
                "Class_{idx}-{name} Result: iou/accuracy {iou:.4f}/{accuracy:.4f} | Dice/F1 {dice:.4f} | Precision {prec:.4f} | Recall {rec:.4f}".format(
                    idx=i,
                    name=self.trainer.cfg.data.names[i],
                    iou=iou_class[i],
                    accuracy=acc_class[i],
        ...
        if self.trainer.writer is not None:
            ...
            self.trainer.writer.add_scalar("val/mAcc", m_acc, current_epoch)
            self.trainer.writer.add_scalar("val/allAcc", all_acc, current_epoch)
            self.trainer.writer.add_scalar("val/meanDice", mean_dice, current_epoch)
            self.trainer.writer.add_scalar("val/meanPrecision", mean_precision, current_epoch)
            self.trainer.writer.add_scalar("val/meanRecall", mean_recall, current_epoch)
                ...
                wandb.log(
                    {
                        ...
                        "val/mAcc": m_acc,
                        "val/allAcc": all_acc,
                        "val/meanDice": mean_dice,
                        "val/meanPrecision": mean_precision,
                        "val/meanRecall": mean_recall,
                    },
                    step=wandb.run.step,
                )
            if self.write_cls_iou:
                for i in range(self.trainer.cfg.data.num_classes):
                    self.trainer.writer.add_scalar(
                        f"val/cls_{i}-{self.trainer.cfg.data.names[i]} IoU", iou_class[i], current_epoch,
                    )
                    self.trainer.writer.add_scalar(
                        f"val/cls_{i}-{self.trainer.cfg.data.names[i]} Dice", dice[i], current_epoch,
                    )
                    self.trainer.writer.add_scalar(
                        f"val/cls_{i}-{self.trainer.cfg.data.names[i]} Precision", precision[i], current_epoch,
                    )
                    self.trainer.writer.add_scalar(
                        f"val/cls_{i}-{self.trainer.cfg.data.names[i]} Recall", recall[i], current_epoch,
                    )
                if self.trainer.cfg.enable_wandb:
                    for i in range(self.trainer.cfg.data.num_classes):
                        wandb.log(
                            {
                                "Epoch": current_epoch,
                                f"val/cls_{i}-{self.trainer.cfg.data.names[i]} IoU": iou_class[i],
                                f"val/cls_{i}-{self.trainer.cfg.data.names[i]} Dice": dice[i],
                                f"val/cls_{i}-{self.trainer.cfg.data.names[i]} Precision": precision[i],
                                f"val/cls_{i}-{self.trainer.cfg.data.names[i]} Recall": recall[i],
                            },
                            step=wandb.run.step,
                        )
        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
        ...
        self.trainer.comm_info["metrics"] = {
            "mIoU": m_iou,
            "meanDice": mean_dice,
    ...
    def after_train(self):
        # self.trainer.best_metric_value = self.trainer.best_metrics["mIoU"]
        # self.trainer.logger.info(
        #     "Best {}: {:.4f}".format("mIoU", self.trainer.best_metric_value)
        # )
        for metric, value in self.trainer.best_metrics.items():
            self.trainer.logger.info(f"Best {metric}: {value:.4f}")
