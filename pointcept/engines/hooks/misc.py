    ...
    def after_epoch(self):
        if is_main_process():
            if self.trainer.cfg.evaluate:
                metrics = self.trainer.comm_info.get("metrics", {})
                model_dir = os.path.join(self.trainer.cfg.save_path, "model")
                # Track which metric is the "main" metric for model_best.pth
                main_metric = "mIoU"  # <- Change this if you want Dice or another metric as primary
    
                # Save per-metric best files and track if main metric is updated
                main_metric_updated = False
                for metric_name, current_value in metrics.items():
                    best_value = self.trainer.best_metrics.get(metric_name, float('-inf'))
                    if current_value > best_value:
                        self.trainer.best_metrics[metric_name] = current_value
                        self.trainer.logger.info(
                            f"Best validation {metric_name} updated to: {current_value:.4f}"
                        )
                        # Save a best file for this metric
                        metric_best_file = os.path.join(model_dir, f"model_best_{metric_name}.pth")
                        torch.save(
                            {
                                "epoch": self.trainer.epoch + 1,
                            ...
                            metric_best_file + ".tmp",
                        )
                        os.replace(metric_best_file + ".tmp", metric_best_file)
                        if metric_name == main_metric:
                            main_metric_updated = True
    
                # Always save the last checkpoint
                last_file = os.path.join(model_dir, "model_last.pth")
                self.trainer.logger.info("Saving checkpoint to: " + last_file)
                torch.save(
                    {
                        "epoch": self.trainer.epoch + 1,
                        "state_dict": self.trainer.model.state_dict(),
                        ...
                        "best_metric_value": max(self.trainer.best_metrics.values()),
                    },
                    last_file + ".tmp",
                )
                os.replace(last_file + ".tmp", last_file)
    
                # Copy the best model by main metric to model_best.pth for compatibility
                main_metric_best_file = os.path.join(model_dir, f"model_best_{main_metric}.pth")
                model_best_file = os.path.join(model_dir, "model_best.pth")
                # Only copy if the metric was updated OR if model_best.pth does not yet exist
                if (main_metric_updated or not os.path.exists(model_best_file)) and os.path.exists(main_metric_best_file):
                    shutil.copyfile(main_metric_best_file, model_best_file)
    
                # Optionally save epoch checkpoints
                if self.save_freq and (self.trainer.epoch + 1) % self.save_freq == 0:
                    epoch_file = os.path.join(
                        model_dir, f"epoch_{self.trainer.epoch + 1}.pth"
                    )
                    shutil.copyfile(last_file, epoch_file)
