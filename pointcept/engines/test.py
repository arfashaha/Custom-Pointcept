...
class SemSegTester(TesterBase):
    def test(self):
        ...
        if (
            ...
            or self.cfg.data.test.type == "CustomTwoClassDataset"
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            dice_class = 2 * intersection / (union + target + 1e-10)
            precision_class = intersection / (union + 1e-10)
            recall_class = intersection / (target + 1e-10)
            dice = np.mean(dice_class[mask])
            recall = np.mean(recall_class[mask])
            precision = np.mean(precision_class[mask])

            m_iou = np.mean(intersection_meter.sum / (union_meter.sum + 1e-10))
            m_acc = np.mean(intersection_meter.sum / (target_meter.sum + 1e-10))
            m_Dice = np.mean(2 * intersection_meter.sum / (union_meter.sum + target_meter.sum + 1e-10))
            m_Precision = np.mean(intersection_meter.sum / (union_meter.sum + 1e-10))
            m_Recall = np.mean(intersection_meter.sum / (target_meter.sum + 1e-10))
            ...
            "Dice {dice:.4f} ({meanDice:.4f}) "
            "Precision {precision:.4f} ({meanPrecision:.4f}) "
            "Recall {recall:.4f} ({meanRecall:.4f})".format(
                data_name,
                idx + 1,
                len(self.test_loader),
                segment.size,
                ...
                iou=iou,
                m_iou=m_iou,
                dice=dice,            # Per-scene dice, if you compute it
                meanDice=m_Dice,    # Cumulative mean Dice
                precision=precision,  # Per-scene precision
                meanPrecision=m_Precision,
                recall=recall,        # Per-scene recall
                meanRecall=m_Recall,
            )
        )
        ...
        if comm.is_main_process():
            print("enter comm.is_main_process()")
            record = {}
            ...
            logger.info(
                "Val result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f} | Dice/F1 {:.4f} | "
                "Precision {:.4f} | Recall {:.4f}".format(
                    mIoU, mAcc, allAcc, mDice, mPrecision, mRecall
                )
            )
            ...
            for i in range(self.cfg.data.num_classes):
                logger.info(
                    "Class_{idx} - {name} Result: iou/acc {iou:.4f}/{acc:.4f} | "
                    "Dice/F1 {dice:.4f} | Precision {prec:.4f} | Recall {rec:.4f}".format(
                        idx=i,
                        name=self.cfg.data.names[i],
                        iou=iou_class[i],
                        acc=accuracy_class[i],
                        dice=dice_class[i],
