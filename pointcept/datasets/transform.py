...
def print_shapes(data_dict, tag):
    try:
        print(f"{tag}: coord {data_dict['coord'].shape}, normal {data_dict['normal'].shape}")
    except Exception as e:
        print(f"{tag}: Could not print shapes ({e})")
    ...
    def __call__(self, data_dict):
        ...
        if self.mode == "train":  # train mode
            ...
            if self.return_displacement:
                # <== FIX: recompute for reduced arrays
                scaled_coord = data_dict["coord"] / np.array(self.grid_size)
                grid_coord = np.floor(scaled_coord).astype(int)
...
class Compose(object):
    ...
    def __call__(self, data_dict):
        for t in self.transforms:
            # print_shapes(data_dict, t.__class__.__name__ + " (before)")
            # data_dict = t(data_dict)
            # print_shapes(data_dict, t.__class__.__name__ + " (after)")
            data_dict = t(data_dict)
        return data_dict
